    ...
    public function run(): void
    {
        ...
        $tenants = [
            [
                ...
                'admin'     => ['name' => 'Admin', 'last_name' => 'Demo', 'email' => 'admin@demo.eduwanka.com'],
                'professor' => ['name' => 'Carlos', 'last_name' => 'Mendoza Quispe', 'email' => 'prof@demo.eduwanka.com'],
                'students'  => [
            ...
            [
                ...
                'admin'     => ['name' => 'Admin', 'last_name' => 'Azul', 'email' => 'admin@azul.eduwanka.com'],
                'professor' => ['name' => 'Elena', 'last_name' => 'Vargas Torres', 'email' => 'prof@azul.eduwanka.com'],
                'students'  => [
            ...
            [
                ...
                'admin'     => ['name' => 'Admin', 'last_name' => 'Verde', 'email' => 'admin@verde.eduwanka.com'],
                'professor' => ['name' => 'Javier', 'last_name' => 'Huamán Condori', 'email' => 'prof@verde.eduwanka.com'],
                'students'  => [
        ...
        $this->command->info('║     URL:    demo.eduwanka.com                                ║');
        $this->command->info('║     Admin:  admin@demo.eduwanka.com / demo1234              ║');
        $this->command->info('║     Prof:   prof@demo.eduwanka.com  / demo1234              ║');
        $this->command->info('║     Color:  #7A0F1F (Granate) + #C8A14A (Dorado)            ║');
        $this->command->info('║                                                              ║');
        $this->command->info('║  🔵 Instituto Azul (Navy)                                   ║');
        $this->command->info('║     URL:    azul.eduwanka.com                                ║');
        $this->command->info('║     Admin:  admin@azul.eduwanka.com / demo1234              ║');
        $this->command->info('║     Prof:   prof@azul.eduwanka.com  / demo1234              ║');
        $this->command->info('║     Color:  #0F3D7A (Azul Navy) + #4A98C8 (Celeste)         ║');
        $this->command->info('║                                                              ║');
        $this->command->info('║  🟢 Centro Verde (Esmeralda)                                ║');
        $this->command->info('║     URL:    verde.eduwanka.com                               ║');
        $this->command->info('║     Admin:  admin@verde.eduwanka.com / demo1234             ║');
        $this->command->info('║     Prof:   prof@verde.eduwanka.com  / demo1234             ║');
    ...
    private function seedTenant(array $config): void
    {
        ...
        foreach ($config['students'] as $idx => [$name, $lastName]) {
            $studentIds[] = DB::table('users')->insertGetId([
                'name'       => $name,
                'last_name'  => $lastName,
                'email'      => "est{$idx}@{$config['slug']}.eduwanka.com",
                'password'   => Hash::make('demo1234'),
                'role'       => 'student',
                'tenant_id'  => $tenantId,
